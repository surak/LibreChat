...
jest.mock('~/data-provider', () => ({
  ...
  useGetStartupConfig: jest.fn(() => ({
    data: { copyTextAppend: 'Text generated by AI on Blablador' },
  })),
}));
...
`;
  ...
  describe('copyTextAppend configuration', () => {
    it('should append configured text for AI messages (isCreatedByUser is false)', () => {
      const { result } = renderHook(() =>
        useCopyToClipboard({
          text: 'AI response',
      ...
      expect(mockCopy).toHaveBeenCalledWith('AI response\n\nText generated by AI on Blablador', {
        format: 'text/plain',
      });
    });

    it('should NOT append configured text for user messages (isCreatedByUser is true)', () => {
      const { result } = renderHook(() =>
        useCopyToClipboard({
          text: 'User message',
          isCreatedByUser: true,
      ...
      expect(mockCopy).toHaveBeenCalledWith('User message', {
        format: 'text/plain',
      });
    });

    it('should append configured text by default (isCreatedByUser is undefined)', () => {
      const { result } = renderHook(() =>
        useCopyToClipboard({
      ...
      expect(mockCopy).toHaveBeenCalledWith('Default response\n\nText generated by AI on Blablador', {
        format: 'text/plain',
      });
    });
  });
